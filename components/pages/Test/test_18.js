...
const Test19 = ({ navigation, route }) => {
        ...
        return (
            <View>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'flex-end' }}>
                    <Text style={{ fontSize: 22 }} >18</Text>
                    <Text style={{ fontSize: 18 }}> / {allQuestions.length}</Text>
                </View>

                {/* question */}
                <Text style={{ fontSize: 30, fontWeight: 'bold' }}>{allQuestions[17]?.question}</Text>
            </View>
        )
    ...
    const checkedOption1 = () => {
        ...
        navigation.navigate('Test19', { next: 0.9 })
    }
    ...
    const backButton = () => {
        return (
            <TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()} onPressIn={() => { setBar(0.75) }} >
                <Image source={arrow} style={{ resizeMode: 'contain', height: 25 }} />
            </TouchableOpacity>
        )
    }
    
    const [bar, setBar] = useState(0.8)
