...
const Test17 = ({ navigation, route }) => {
        ...
        return (
            <View>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'flex-end' }}>
                    <Text style={{ fontSize: 22 }} >17</Text>
                    <Text style={{ fontSize: 18 }}> / {allQuestions.length}</Text>
                </View>

                {/* question */}
                <Text style={{ fontSize: 30, fontWeight: 'bold' }}>{allQuestions[16]?.question}</Text>
            </View>
        )
    ...
    const checkedOption1 = () => {
        ...
        navigation.navigate('Test18', { next: 0.85 })
    }
        ...
        return (
            <View>
                ...
                <TouchableOpacity style={[styles.option, { backgroundColor: optionColor5 }]} onPress={checkedOption5}>
                    <Text style={{ fontSize: 18 }}>Benci</Text>
                </TouchableOpacity>
            </View>
        )
    }
    ...
    const backButton = () => {
        return (
            <TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()} onPressIn={() => { setBar(0.7) }} >
                <Image source={arrow} style={{ resizeMode: 'contain', height: 25 }} />
            </TouchableOpacity>
        )
    }
    
    const [bar, setBar] = useState(0.75)
