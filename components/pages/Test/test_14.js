...
const Test14 = ({ navigation, route }) => {
    ...
    const renderQuestion = () => {
        return (
            <View>
                ...
                {/* question */}
                <Text style={{ fontSize: 30, fontWeight: 'bold' }}>{allQuestions[13]?.question}</Text>
            </View>
        )
    ...
    const checkedOption1 = () => {
        ...
        navigation.navigate('Test15', { next: 0.7 })
    }
    ...
    const backButton = () => {
        return (
            <TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()} onPressIn={() => { setBar(0.55) }} >
